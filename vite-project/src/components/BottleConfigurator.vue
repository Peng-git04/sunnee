    ...
    <div class="left">
      ...
      <template v-if="done">
        <div class="success">
          ...
          <p class="success-msg">
            La tua bottiglia è pronta. Riceverai un regalo speciale
            con il tuo primo ordine Sunnee Kids!
          </p>
          <div class="recap">
            <div v-for="item in recapItems" :key="item.part" class="recap-row">
      ...
      <template v-else>
        ...
        <div class="actions">
          ...
          <button v-if="step < 2" class="btn-primary" @click="step++">
            Avanti →
          </button>
          <button v-else class="btn-send" @click="done = true">
            Invia 🎉
          </button>
        </div>
      </template>
...
const chosen = ref<Chosen>([6, 5, 4])
const done   = ref<boolean>(false)

function selectColor(idx: number): void {
  const next = [...chosen.value] as Chosen
  next[step.value] = idx
  chosen.value = next
}
...
.brand {
  ...
  font-size: 22px; font-weight: 800; color: #A29BFE; margin-bottom: 2px;
}
...
.btn-ghost {
  ...
  font-size: 14px; cursor: pointer; color: #888; transition: background .2s;
}
...
.recap {
  ...
  padding: 14px 20px; width: 100%; max-width: 280px; margin-bottom: 20px;
}
.recap-row {
  display: flex; align-items: center; gap: 9px;
  font-size: 13px; font-weight: 700; color: #444; margin-bottom: 7px;
}
